...
DBAppendCmd::exec(const string& option)
{
   ...
   if (doRow) {
	vector<int> dbAppendRow;
	for(int i=1;i<= (int)dbtbl.nCols();i++)	
	{
			int data;
		if( i>= (int)options.size() || options[i]=="-")	dbAppendRow.push_back(INT_MAX);
		else if(!myStr2Int(options[i],data))	
		{
			cout<<"Error: \""<<options[i]<<"\" is not an integer!!"<<endl;
			insertFlag = false;	
		}
		else	
		{
			dbAppendRow.push_back(data);
		}
	}
	if(dbtbl.nCols() == 0)
	{
		...
		{
			if(options[j]=="-")	dbAppendRow.push_back(INT_MAX);
			else if(!myStr2Int(options[j],data))	
			{
				cout<<"Error: \""<<options[j]<<"\" is not an integer!!"<<endl;
				insertFlag = false;	
			}
			else	
			{
	...
	DBRow singleRow(dbAppendRow);
	if(insertFlag)	dbtbl.addRow(singleRow); 
   }
   else 
   {
	insertFlag = true;
	vector<int> dbAppendCol;
	for(int i=1;i<= (int)dbtbl.nRows();i++)	
	{
		int data;
		if(i>=(int)options.size() || options[i]=="-")	dbAppendCol.push_back(INT_MAX);	
		else if(!myStr2Int(options[i],data))	
		{
			cout<<"Error: \""<<options[i]<<"\" is not an integer!!"<<endl;
			insertFlag = false;	
	...
	if(dbtbl.nRows() == 0)
	{
		for(int j=1;j< (int)options.size();j++)	
		{
			cout<<j<<dbtbl.nRows();	
			int data;
			if( options[j]=="-")	dbAppendCol.push_back(INT_MAX);	
			else if(!myStr2Int(options[j],data))	
			{
				cout<<"Error: \""<<options[j]<<"\" is not an integer!!"<<endl;
				insertFlag = false;	
	...
	if(insertFlag)	dbtbl.addCol(dbAppendCol); 
 }
	...
	if(options.size() == 2)
	{
		if(myStr2Int(options[0],rowIdx))
		{
			if(myStr2Int(options[1],colIdx))
			{
				...
				{
					if(dbtbl.getData(rowIdx,colIdx)!= INT_MAX)	cout<<dbtbl.getData(rowIdx,colIdx)<<endl;
					else	cout<<"null"<<endl;
				}
			}
		}	
		...
		{
			if( options.size() == 1 || !checkRowIdx(options[1],rowIdx))	return CMD_EXEC_ERROR;
			cout<<dbtbl[rowIdx];
		}	
  		else if (myStrNCmp("-Column", options[0], 2) == 0)
		{	
			if( options.size() == 1 || !checkColIdx(options[1],colIdx))	return CMD_EXEC_ERROR;
   	...
   	else if(myStrNCmp("-Summary", options[0], 2) == 0)	cout<<"(#rows, #cols, #data) = ("<<dbtbl.nRows()<<", "<<dbtbl.nCols()<<", "<<dbtbl.countNonNullData()<<")"<<endl;
