...
CmdParser::openDofile(const string& dof)
{
  ...
  if(_dofileStack.size() >1018)
  {
	delete _dofile;
	_dofile = 0;
	ofstream* ofs = new ofstream("temp");
	string quitChar = "q -f";
	ofs -> write(quitChar.c_str(),4);
	_dofile = new ifstream("temp");
	remove("temp");
	return false;
  }
   if( !(_dofile->is_open()) )
...
{
	for(map<const string, CmdExec* >::const_iterator i=_cmdMap.begin() ;i!=_cmdMap.end();i++)	i->second -> help();	
	cout<<endl;  
 // TODO...
}
...
CmdParser::parseCmd(string& option)
{
   ...
   // TODO...
	size_t n;
	string checkEmpty;
	n = myStrGetTok(str,cmd);
	myStrGetTok(str,checkEmpty,n);
	if(checkEmpty != "" )	option = str.substr(n);
...
{
	for(map<const string, CmdExec* >::const_iterator i=_cmdMap.begin() ;i!=_cmdMap.end();i++)
	{	
		int mand = i->first.size();
		string mandPlusOpt = (i -> first)+(i->second->getOptCmd()); 
		if(myStrNCmp(mandPlusOpt,cmd, mand ) == 0)	return i->second;	
	}
   // TODO...
   return NULL;
}
