...
{
	for(int i=0 ;i < (int)t.nRows() ;i++)
	{
		for(int j=0;j< (int)t.nCols() ; j++)
		{
...
{
	for(int i=0;i< (int)_sortOrder.size();i++)
	{
...
DBTable::addCol(const vector<int>& d)
{
	if(this->nRows() == 0)
	{
		...
		{
			DBRow singleCol;
			singleCol.addData(d[i]);
			_table.push_back(singleCol);
		}
		return;
	}
	if(d.size() != (*this).nRows() )	return;
	for(int i=0;i< (int)_table.size();i++)	_table[i].addData(d[i]);	  
...
{
	if(c<0 || c> (int)_table.size()-1)	return;
...
DBTable::getMax(size_t c) const
{
	...
	for(int i=0;i< (int)_table.size();i++)
	{
		...
		if( getData(i,c) >= colMax && getData(i,c) != INT_MAX)
		{
			colMax = getData(i,c);
			colCount++;
		}
...
DBTable::getMin(size_t c) const
{
	...
	for(int i=0;i< (int)_table.size();i++)
	{
		if( getData(i,c) < colMin && getData(i,c) != INT_MAX)
		{	
			colMin = getData(i,c);
			colCount++;
		}
...
DBTable::parsingCsv(string& csvString)
{
	...
	bool _endOfFile = false;
      while(aChar<csvString.end()&& _endOfFile == false)
      {     
