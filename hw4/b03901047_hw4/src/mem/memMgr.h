...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {						
      // TODO
	ret = (T*)_ptr;	
	if((size_t)(_end - _ptr) < t)	return false;
	_ptr = _ptr + toSizeT(t);
      return true;
   }
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
	MemRecycleList<T>* _seekRecycleList = &_recycleList[m];
	while(_seekRecycleList != 0)
	{
		if(_seekRecycleList -> getArrSize() == n)
		{	
			return _seekRecycleList;	
		}	
		if(_seekRecycleList -> _nextList) _seekRecycleList = _seekRecycleList -> _nextList;
		else break;
	}
	MemRecycleList<T>* newList = new MemRecycleList<T>(n);
	_seekRecycleList -> setNextList(newList);
	return  _seekRecycleList -> _nextList;
   }
   ...
   T* getMem(size_t t) {
	...
	MemRecycleList<T>* gotList = getMemRecycleList(n);	
	...
	else
	{
		...
		{
			size_t remainBlockSize = _activeBlock -> getRemainSize();
          		
			if(remainBlockSize >= toSizeT(S))
			{
				size_t rn = getArraySize(downtoSizeT(remainBlockSize));
				#ifdef MEM_DEBUG
	          		cout << "Recycling " << ret << " to _recycleList[" << rn << "]\n";
	          		#endif //MEM_DEBUG
				getMemRecycleList(rn) -> pushFront(ret);		
			}
			MemBlock<T>* _secondBlock = _activeBlock;
			_activeBlock = new MemBlock<T>(_secondBlock,_blockSize);
         		 #ifdef MEM_DEBUG
   ...
   size_t getNumBlocks() const {
	...
	MemBlock<T>* _countBlock = _activeBlock;
	while(_countBlock)	
	{
		_blockCount++;
		_countBlock = _countBlock -> getNextBlock();
	}
      return _blockCount;
   }
